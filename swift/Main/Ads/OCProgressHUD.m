...
@interface OCProgressHUD ()

@property (nonatomic, weak) MBProgressHUD *progressHUD;

@end

@implementation OCProgressHUD

#pragma mark - class method
+ (instancetype)sharedHUD {
    static OCProgressHUD *hud = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!hud) {
            hud = [OCProgressHUD new];
        }
    });
    return hud;
}
...
+ (void)showProgressHUDWithMode:(ProgressHUDMode)mode withText:(NSString *)text afterDelay:(NSTimeInterval)delay isTouched:(BOOL)touched inView:(UIView *)view {
    [[OCProgressHUD sharedHUD] showProgressHUDWithMode:mode withText:text afterDelay:delay isTouched:touched inView:view];
}

+ (void)showProgressHUDWithMode:(ProgressHUDMode)mode withText:(NSString *)text isTouched:(BOOL)touched inView:(UIView *)view {
    [[OCProgressHUD sharedHUD] showProgressHUDWithMode:mode withText:text isTouched:touched inView:view];
}

+ (void)hideProgressHUDAfterDelay:(NSTimeInterval)delay {
    [[OCProgressHUD sharedHUD] hideProgressHUDAfterDelay:delay];
}

+ (void)setProgress:(CGFloat)progress {
    [[OCProgressHUD sharedHUD] setProgress:progress];
}
...
{
    [[OCProgressHUD sharedHUD]showProgressHUDInView:view withText:text afterDelay:delay];
}

+ (void)showProgressTipHUDInView:(UIView *)view withText:(NSString *)text afterDelay:(NSTimeInterval)delay
{
    [[OCProgressHUD sharedHUD]showProgressTipHUDInView:view withText:text afterDelay:delay];
}
...
{
    return [[OCProgressHUD sharedHUD]UpIMGShowProgressHUDWithText:text isTouched:touched inView:view progressTintColor:progressTintColor];
}
- (MBProgressHUD *)UpIMGShowProgressHUDWithText:(NSString *)text isTouched:(BOOL)touched inView:(UIView *)view progressTintColor:(UIColor *)progressTintColor {
    if (view == nil) {
        view = [UIApplication sharedApplication].keyWindow;
...
- (void)showProgressTipHUDInView:(UIView *)view withText:(NSString *)text afterDelay:(NSTimeInterval)delay{
    ...
    progressHUD.detailsLabel.font = [UIFont systemFontOfSize:18];
    progressHUD.mode = MBProgressHUDModeText;
    progressHUD.removeFromSuperViewOnHide = YES;
    [self.progressHUD hideAnimated:YES afterDelay:delay];
}
