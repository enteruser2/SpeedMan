...
NSTimeInterval const HUD_DismisTime = 1.5;
...
{
    [[ProgressHUD sharedHUD]showProgressTipHUDInView:view withText:text afterDelay:delay];
}
...
- (void)showProgressTipHUDInView:(UIView *)view withText:(NSString *)text afterDelay:(NSTimeInterval)delay{
    ...
    // 多行显示
    progressHUD.detailsLabelText = text;
    progressHUD.detailsLabelFont = [UIFont systemFontOfSize:18];
    progressHUD.mode = MBProgressHUDModeText;
    progressHUD.removeFromSuperViewOnHide = YES;
    [progressHUD hide:YES afterDelay:delay];
}
